...
`Kliiniset tilanteet` presents one complete, structured anamnesis at a time (V1 ships one case, Rintakipu — chest pain), organized into eight fixed sections (nykyinen vaiva, liitännäisoireet, aiemmat sairaudet ja toimenpiteet, lääkitys ja allergiat, sukuanamneesi, tupakka/alkoholi/päihteet, sosiaalinen anamneesi ja elintavat, perustiedot) covering 39 questions in total. The section sequence is fixed and always shown, never shuffled: the ordering is itself part of what the learner is meant to internalize.

For each item the learner reads one Swedish patient line, types their own Swedish question for `Mitä kysyt potilaalta?`, and reveals one or more Swedish model questions to compare against. The typed answer is never automatically graded — several phrasings can be equally correct — so the learner self-assesses with `Osasin` or `En osannut` before continuing. A section boundary crossing moves focus to the section indicator so screen reader users notice the structural change, not just the next question.

The typed session persists its case, current item position, in-progress draft answer, reveal/self-assessment state for the current item, and every finished item's self-assessment under its own browser-storage key. A stored session that fails validation (unknown case, tampered shape, or a leftover session from the retired multiple-choice experience) is treated as recoverable, not fatal: it is discarded and a fresh session starts automatically rather than dead-ending the learner. Each finished item counts as one unique item toward XP, the daily goal, and quest variety through the same `session-started` / `item-completed` / `session-completed` / `active-study` events as the other exercises, using the `clinical` exercise mode — it does not use, and is not used by, the flashcard retry/mastery or Smart Review word-stats engine. Completion reports how many items were known versus not and elapsed time; `Uusi kierros` restarts the same case with a new identifier and empty progress state.

The content model (`AnamnesisCase` → `AnamnesisSection` → `AnamnesisItem`) is deliberately generic so additional cases (for example vatsakipu, hengenahdistus, kuume, päänsärky) can reuse the same engine; only Rintakipu ships in V1.
...
### Add an anamnesis case

Add an object to `content/anamnesis-cases.json` with a unique lowercase ID, a Finnish name, and exactly eight sections in the fixed required order (see `requiredAnamnesisSections` in `scripts/validate-content.ts`), each with its own required item count. Every item needs a sequential `<caseId>-NN` ID, one natural Swedish patient line, and one to three Swedish model questions (either alternative phrasings of the same question, or — as with the closing `perustiedot` item — a few closely related questions bundled under one patient answer).

Keep dialogue natural Finland-Swedish clinical register (`ni`, not `du`); slash-separated alternatives, semicolons, placeholder text, and duplicate model questions within an item are rejected. This content is intentionally exact and reviewed as a unit, not grown incrementally — `tests/unit/anamnesis-content.test.ts` locks the full required text for the shipped case. Run content validation and all tests before committing.

## Tests and accessibility

Vitest covers directions, duration formatting, deterministic unique session and new-round selection, delayed phrase recall, summary statistics, typed transitions, URL matching, strict deck- and category-aware stored-session validation, answer normalisation, articles and inflections, malformed content, and — for the anamnesis case specifically — exact section/item ordering and required content, free-text draft persistence, reveal and self-assessment transitions, and stored-session validation including tampered/unknown-case rejection. Playwright uses controlled clocks and randomness for elapsed timing and stable selections, and covers full-row activation, focus transitions, refresh-stable sessions and drafts, retries, new rounds, invalid URLs, all session sizes, mobile control sizing, responsive overflow, visual QA, and serious/critical axe violations, including a full 39-item anamnesis walkthrough across all eight sections, its reveal/self-assessment states, section-change focus, and shared progress/motion/sound event integration.
...
The mobile homepage is a compact launcher rather than a dashboard. After the global header and four-item HUD it contains one full-width daily launcher followed immediately by the four Finnish practice actions. The persistent HUD shows level with XP, streak, credits, and unopened rewards on non-active routes; rewards link directly to opening and receive restrained emphasis when available. Season and league detail stay on their existing pages.
