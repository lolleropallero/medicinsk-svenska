...
let installed=false,unlocked=false,playing=false,crossfading=false,activeIndex=0,ducked=false,lastDuckAt=-Infinity,restoreTimer=0,frame=0;
...
async function begin(){
  if(!unlocked||playing||!loadMusicSettings().enabled||loadMusicSettings().volume<=0)return;
  ...
  try{await active.audio.play();playing=true;prepareNext('metadata');persist();}catch{playing=false;}
}
...
async function advance(useFade=true){
  ...
  try{incoming.audio.currentTime=0;incoming.mix=0;setGains();await incoming.audio.play();}catch{markFailed(next);return;}
  crossfading=true;const duration=useFade?CROSSFADE_SECONDS*1000:120,started=performance.now();
  const tick=(now:number)=>{const progress=Math.min(1,(now-started)/duration),mix=equalPowerCrossfade(progress);outgoing.mix=mix.outgoing;incoming.mix=mix.incoming;setGains();if(progress<1)requestAnimationFrame(tick);else finishAdvance(outgoing,incoming,next);};requestAnimationFrame(tick);
}
...
function monitorPlayback(){
  if(!playing||crossfading||!session)return;const active=channels[activeIndex]!,remaining=active.audio.duration-active.audio.currentTime;if(Number.isFinite(remaining)&&remaining<=15)prepareNext('auto');if(Number.isFinite(remaining)&&remaining<=CROSSFADE_SECONDS&&active.audio.currentTime>0)void advance(true);persist();
}
function pauseSmooth(){playing=false;const active=channels[activeIndex];if(!active)return;active.mix=0;rampGains(200);window.setTimeout(()=>{channels.forEach(channel=>channel.audio.pause());persist();},220);}
...
export function startMusicPlayer(host:HTMLElement=document.getElementById('music-player')!){
  ...
  const unlock=()=>{if(unlocked)return;unlocked=true;try{sessionStorage.setItem(UNLOCK_KEY,'true');}catch{}void begin();};window.addEventListener('pointerdown',unlock,{capture:true});window.addEventListener('keydown',unlock,{capture:true});window.addEventListener('music-settings-changed',onSettings);window.addEventListener('sound-effect-requested',onSound as EventListener);window.addEventListener('pagehide',persist);window.addEventListener('progress-updated',()=>rampGains(200));
