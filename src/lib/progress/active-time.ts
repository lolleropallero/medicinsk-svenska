...
export function startActiveTime(options:ActiveTimeOptions){
  const tabId=crypto.randomUUID(),now=options.now??Date.now,storage=options.storage??localStorage;let lastInteraction=now(),lastAccounted=now(),sequence=0;
  ...
  const timer=window.setInterval(flush,10_000);['pointerdown','keydown','input'].forEach(type=>document.addEventListener(type,interact,{passive:true}));document.addEventListener('visibilitychange',flush);window.addEventListener('pagehide',flush);
  return {interact,flush,stop(){window.clearInterval(timer);flush();try{const lease=JSON.parse(storage.getItem(LEASE_KEY)??'null');if(lease?.tabId===tabId)storage.removeItem(LEASE_KEY);}catch{/* optional */}}};
}
