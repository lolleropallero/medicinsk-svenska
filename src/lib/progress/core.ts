...
export const LEAGUE_TIERS=['Pronssi','Hopea','Kulta','Platina','Timantti','Konsultti'] as const;
...
export function reconcileProgress(input:ProgressStateV1,now=Date.now()):ProgressStateV1{const state=structuredClone(input),today=localDayKey(now);let changed=false;
  ...
  const currentWeek=localWeekKey(now);if(currentWeek>state.league.weekKey&&!state.league.settledWeeks.includes(state.league.weekKey)){const old=state.league.tier,index=LEAGUE_TIERS.indexOf(old),xp=state.league.weeklyXp;let next=index,kind:'retained'|'promoted'|'demoted'='retained';if(xp>=LEAGUE_PROMOTION_XP[index]!&&index<LEAGUE_TIERS.length-1){next=index+1;kind='promoted';addCapsule(state,next===1?'standard':'golden',`league:${state.league.weekKey}`,now);}else if(xp<LEAGUE_RETENTION_XP[index]!&&index>0){next=index-1;kind='demoted';}else if(old==='Konsultti')addCapsule(state,'golden',`league:${state.league.weekKey}:retain`,now);else state.inventory.credits+=10*(index+1);state.league.tier=LEAGUE_TIERS[next]!;state.league.result={kind,tier:LEAGUE_TIERS[next]!};delete state.league.previousResult;state.notifications.push({id:`league:${state.league.weekKey}:result`,kind:'league',result:state.league.result});state.league.settledWeeks.push(state.league.weekKey);state.league.weekKey=currentWeek;state.league.weeklyXp=0;changed=true;}
