...
import { requestFeedback } from '../motion/feedback';
...
export function dispatchProgress(event:ProgressEvent,storage:Storage=localStorage){const current=loadProgress(storage,event.occurredAt);const result=reduceProgress(current,event);if(result.applied){saveProgress(result.state,storage);const priorNotifications=new Set(current.notifications.map(item=>item.id));const added=result.state.notifications.filter(item=>!priorNotifications.has(item.id));const effect=levelProgress(result.state.lifetime.xp).level>levelProgress(current.lifetime.xp).level?'level-up':added.some(item=>item.kind==='achievement')?'achievement':added.some(item=>item.kind==='daily-quest'||item.kind==='weekly-quest')?'quest-complete':null;if(effect)requestFeedback(effect);}return result;}
