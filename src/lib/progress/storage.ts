...
import type { ProgressEvent, ProgressStateV1 } from './types';
...
export function loadProgress(storage: Pick<Storage,'getItem'|'setItem'|'removeItem'>=localStorage, now=Date.now()): ProgressStateV1 {
  ...
  try { const parsed:unknown=JSON.parse(raw);if(isProgressState(parsed)){const reconciled=reconcileProgress(parsed,now);if(reconciled.updatedAt!==parsed.updatedAt)storage.setItem(PROGRESS_KEY,JSON.stringify(reconciled));return reconciled;}storage.setItem(`${PROGRESS_KEY}.corrupt.${now}`,raw); }
...
export function parseImport(raw:string):{ok:true;state:ProgressStateV1}|{ok:false;error:string}{try{const value:unknown=JSON.parse(raw);if(!value||typeof value!=='object'||Array.isArray(value))return{ok:false,error:'Tiedosto ei ole kelvollinen.'};const envelope=value as Partial<ExportEnvelope>;
  if(envelope.format!=='medicinsk-svenska-progress'||envelope.exportFormatVersion!==1||!isProgressState(envelope.progress))return{ok:false,error:'Tiedoston versio tai sisältö ei kelpaa.'};return{ok:true,state:envelope.progress};}catch{return{ok:false,error:'Tiedostoa ei voitu lukea.'};}}
