...
import { deriveMilestoneBatch, enqueueMilestoneBatch, milestoneNotificationIds } from './milestones';
...
export function dispatchProgress(event:ProgressEvent,storage:Storage=localStorage){
  const current=loadProgress(storage,event.occurredAt);const result=reduceProgress(current,event);
  if(result.applied){
    const batch=deriveMilestoneBatch(current,result.state,event.eventId);
    if(batch){
      const ownedNotifications=milestoneNotificationIds(batch);
      result.state.notifications=result.state.notifications.filter(item=>!ownedNotifications.has(item.id));
    }
    saveProgress(result.state,storage);
    if(batch)enqueueMilestoneBatch(batch);
  }
  return result;
}
