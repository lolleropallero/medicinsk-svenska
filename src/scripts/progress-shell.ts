...
import { claimableSeasonCount, levelProgress } from '../lib/progress/core';
...
function hud(){const state=loadProgress(),host=document.getElementById('metagame-hud');if(!host)return;const level=levelProgress(state.lifetime.xp),boxes=state.inventory.capsules.filter(item=>!item.openedAt).length,claimable=claimableSeasonCount(state);host.innerHTML=`<div class="hud-grid" lang="sv"><a href="/edistyminen/"><span>Nivå</span><strong>${level.level}</strong><small>${state.lifetime.xp} <abbr title="erfarenhetspoäng">XP</abbr></small></a><a href="/edistyminen/"><span>Svit</span><strong>${state.streak.current}</strong><small>${state.streak.current===1?'dag':'dagar'}</small></a><a href="/palkinnot/"><span>Krediter</span><strong>${state.inventory.credits}</strong></a><a class="hud-boxes ${boxes?'has-boxes':''}" href="/palkinnot/#unopened-boxes"><span>Lådor</span><strong>${boxes}</strong>${boxes?'<small>Öppna</small>':''}</a></div>${claimable?`<a class="hud-season" lang="sv" href="/kausi/#reward-track">${claimable} ${claimable===1?'säsongsbelöning väntar':'säsongsbelöningar väntar'}</a>`:''}`;}
