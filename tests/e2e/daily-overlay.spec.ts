...
type DayOptions={goalCount?:number;claimedSlots?:number[];quest?:{kind:string;mode?:string;target:number};modes?:string[];calm?:boolean;lastUsedMode?:string};

async function closeIfOpen(page:Page){const dialog=page.getByRole('dialog',{name:'Dagens uppdrag'});if(await dialog.isVisible())await page.getByRole('button',{name:'Stäng dagens uppdrag'}).click();}
...
test('generic and variety quests resolve to a study session, and reroll remains separate',async({page})=>{
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest:{kind:'items',target:10},lastUsedMode:'phrases'});await page.reload();await openDaily(page);const url=page.url();await page.locator('[data-quest-slot="1"] [data-reroll]').click();await expect(page).toHaveURL(url);await expect(page.getByRole('dialog',{name:'Dagens uppdrag'})).toBeVisible();
