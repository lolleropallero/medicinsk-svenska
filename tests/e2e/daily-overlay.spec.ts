...
test('daily overlay opens once per controlled local day and dismissal is presentation-only',async({page})=>{
  ...
  expect(await page.evaluate(key=>JSON.parse(localStorage.getItem(key)!),UI)).toEqual({schemaVersion:1,soundEnabled:true,soundVolume:.65,musicEnabled:true,musicVolume:.2,dailyOverlayDismissedDay:'2026-08-21'});expect(await page.evaluate(key=>localStorage.getItem(key),PROGRESS)).toBe(before);
