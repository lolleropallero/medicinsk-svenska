...
type DayOptions={goalCount?:number;claimedSlots?:number[];quest?:{kind:string;mode?:string;target:number;answerMode?:string};modes?:string[];calm?:boolean;lastUsedMode?:string;freeRerollUsed?:boolean;rerollTokens?:number};
...
test('daily overlay opens once per controlled local day and dismissal is presentation-only',async({page})=>{
  ...
  expect(await page.evaluate(key=>JSON.parse(localStorage.getItem(key)!),UI)).toEqual({schemaVersion:1,soundEnabled:true,soundVolume:.65,musicEnabled:true,musicVolume:0,dailyOverlayDismissedDay:'2026-08-21'});expect(await page.evaluate(key=>localStorage.getItem(key),PROGRESS)).toBe(before);
...
for(const [label,mode,path,query] of [
  ...
  const quest=mode==='flashcards'?{kind:'mode',mode,target:10,answerMode:'choice'}:{kind:'mode',mode,target:5};
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest});await page.reload();await openDaily(page);if(mode==='flashcards')await expect(page.locator('[data-quest-slot="1"]')).toContainText('flervalsuppgifter');await page.locator('[data-quest-action="1"]').click();await expect.poll(()=>page.url()).toContain(path);expect(page.url()).toContain(query);if(mode==='flashcards')expect(page.url()).toContain('answer=choice');
});

test('generic and variety quests resolve to a study session, and reroll remains separate',async({page})=>{
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest:{kind:'items',target:10,answerMode:'written'},lastUsedMode:'phrases'});await page.reload();await openDaily(page);const url=page.url(),action=page.locator('[data-quest-action="1"]'),reroll=page.locator('[data-quest-slot="1"] [data-reroll]');expect(await action.evaluate(element=>element.tagName)).toBe('BUTTON');await expect(action).toContainText('Starta');expect(await action.locator('.daily-quest-start').evaluate(element=>getComputedStyle(element).backgroundColor)).toBe('rgb(29, 96, 79)');expect(await reroll.evaluate(element=>getComputedStyle(element).backgroundColor)).toBe('rgba(0, 0, 0, 0)');await reroll.click();await expect(page).toHaveURL(url);await expect(page.getByRole('dialog',{name:'Dagens uppdrag'})).toBeVisible();expect(await page.evaluate(key=>localStorage.getItem(key),UI)).toBeNull();
  const answerMode=await page.evaluate(key=>{const state=JSON.parse(localStorage.getItem(key)!);const day=Object.keys(state.daily).sort().at(-1)!;return state.daily[day].quests[0].answerMode;},PROGRESS);
  await page.locator('[data-quest-action="1"]').click();await expect(page).toHaveURL(/\/kortit\/harjoitus\?.*mode=lucky.*amount=10/);expect(page.url()).toContain(`answer=${answerMode}`);
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest:{kind:'variety',target:2,answerMode:'choice'},modes:['phrases','descriptions']});await page.reload();await openDaily(page);await page.locator('[data-quest-action="1"]').click();await expect(page).toHaveURL(/\/kortit\/harjoitus\?.*mode=lucky.*answer=choice/);
...
test('a daily vocabulary mission chooses its own answer mode instead of the stored selection',async({page})=>{
  const storeSession=async(answerMode:'cards'|'choice',sessionId:string)=>page.evaluate(({answerMode,sessionId})=>localStorage.setItem('medicinsk-svenska.flashcard-session.v1',JSON.stringify({schemaVersion:1,sessionId,mode:'lucky',answerMode,direction:'fi-sv',requestedAmount:10,selectedCardIds:['anatomi-001'],unseenCardQueue:[],currentCardId:'anatomi-001',masteredCardIds:[],pendingRetries:[],answerDraft:'',attemptCountByCard:{},firstAttemptCorrectByCard:{},totalMissedCount:0,startedAt:Date.now(),revealed:false})),{answerMode,sessionId});
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest:{kind:'mode',mode:'flashcards',target:10,answerMode:'choice'}});await storeSession('cards','daily-resume-cards');await page.reload();await openDaily(page);await page.locator('[data-quest-action="1"]').click();await expect(page).toHaveURL(/\/kortit\/harjoitus\?.*answer=choice/);expect(page.url()).not.toContain('daily-resume-cards');
  await page.goto('/');await closeIfOpen(page);await seedDay(page,{quest:{kind:'mode',mode:'flashcards',target:10,answerMode:'choice'}});await storeSession('choice','daily-resume-choice');await page.reload();await openDaily(page);await page.locator('[data-quest-action="1"]').click();await expect(page).toHaveURL(/\/kortit\/harjoitus\?.*answer=choice.*session=daily-resume-choice/);
});
