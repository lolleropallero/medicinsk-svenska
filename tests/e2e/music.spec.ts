...
async function makeMusicAudible(page:Page){await page.addInitScript(key=>localStorage.setItem(key,JSON.stringify({schemaVersion:1,soundEnabled:true,soundVolume:.65,musicEnabled:true,musicVolume:.2})),UI);}
...
test('music preferences default independently, persist, fit mobile, and remain accessible',async({page})=>{
  ...
  await expect(music).toBeChecked();await expect(musicVolume).toHaveValue('0');await expect(sound).toBeChecked();
