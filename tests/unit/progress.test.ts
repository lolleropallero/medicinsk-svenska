...
describe('capsules, shop, season and state IO',()=>{
  ...
  it('prioritizes unopened capsules in next action',()=>{const state=createProgressState(at('2026-08-21'),'install');state.inventory.capsules.push({id:'x',kind:'standard',earnedAt:at('2026-08-21')});expect(nextAction(state,at('2026-08-21'))).toEqual({label:'Avaa palkintokapseli',href:'/palkinnot/'});});
});
