...
describe('session URL configuration', () => {
  it('parses a deck request and builds a complete reusable URL', () => {
    ...
    expect(parsed).toEqual({ ok: true, value: {
      mode: 'deck', sourceDeckId: 'anatomi', direction: 'sv-fi', requestedAmount: 10, sessionId: 'readable-1',
    } });
    if (parsed.ok && parsed.value.sessionId) {
      expect(buildSessionUrl(parsed.value as Parameters<typeof buildSessionUrl>[0])).toContain('session=readable-1');
    }
  });

  it('allows a missing session ID for recoverable URL replacement', () => {
    expect(parseSessionRequest('?mode=lucky&direction=fi-sv&amount=all', decks)).toEqual({
      ok: true,
      value: { mode: 'lucky', direction: 'fi-sv', requestedAmount: 'all', sessionId: null },
    });
  });
