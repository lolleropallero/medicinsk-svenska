...
describe('daily overlay UI preferences', () => {
  it('defaults safely when absent or corrupt', () => {
    expect(parseUiPreferences(null)).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20 });
    expect(parseUiPreferences('{broken')).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20 });
    expect(parseUiPreferences('{"schemaVersion":2}')).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20 });
    expect(parseUiPreferences('{"schemaVersion":1,"dailyOverlayDismissedDay":"today"}')).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20 });
  });
  ...
  it('stores dismissal for the controlled local date without touching progress', () => {
    ...
    expect(JSON.parse(storage.getItem(UI_PREFERENCES_KEY)!)).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20, dailyOverlayDismissedDay: '2026-08-21' });
  ...
  it('marks quest activation handled for today and permits automatic opening tomorrow', () => {
    ...
    expect(preferences).toEqual({ schemaVersion: 1, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20, dailyOverlayDismissedDay: '2026-08-21' });
  ...
  it('opens once on an eligible day and becomes eligible on the next local day', () => {
    const preferences = { schemaVersion: 1 as const, soundEnabled:true, soundVolume:.65, musicEnabled:true, musicVolume:.20, dailyOverlayDismissedDay: '2026-08-21' };
    expect(eligible({ preferences })).toBe(false);
    expect(eligible({ localDay: '2026-08-22', preferences })).toBe(true);
  });
